...
const resolvers_Proyectos = {
    Query: {
        listarProyectos: async(parent, arg) => {
            const listadoProyectos = await modeloProyectos.find().populate("Lider_Id").populate("Inscripciones")
            return listadoProyectos
        },
        buscarProyecto: async(parent, arg) => {
            const proyecto = await modeloProyectos.findById({ _id: arg._id }).populate("Lider_Id").populate("Inscripciones")
            return proyecto
        },
        buscarProyectosPorLider: async(parent, arg) => {
            const proyectosPorLider = await modeloProyectos.find({ Lider_Id: arg.Lider_Id }).populate("Lider_Id").populate("Inscripciones")
